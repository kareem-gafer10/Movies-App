...
const CardList = (props) => {
  const {title,name,vote_average,poster_path,profile_path,id,links}=props;
  ...
  return (
    <div className=" col-md-3 gy-5 mx-auto">
      ...
      {poster_path  ?  <img className="w-100 " src={`${imgUrl}${poster_path}`} alt={title} /> :
        ...
        {title ? <h6 className='mt-3  p-2  text-ellipsis'>{title.split(" ").slice(0, 2).join(" ")}</h6> 
        : <h6 className='mt-3  p-2  text-ellipsis'>{name.split(" ").slice(0, 2).join(" ")}</h6> }
        </div>
        </Link>
    </div>
  )
